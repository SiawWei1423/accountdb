<?php
/**
 * AUTO SETUP: Create client_queries table
...
<html>
<head>
    <title>Setup Client Queries Table</title>
    <style>
...
<body>
    <div class='container'>
        <h1>🚀 Setup Client Queries Table</h1>";

// Drop existing table if exists (optional - comment out if you want to keep existing data)
// $conn->query("DROP TABLE IF EXISTS client_queries");

// Check if table already exists
$check_table = $conn->query("SHOW TABLES LIKE 'client_queries'");

if ($check_table->num_rows > 0) {
    echo "<div class='info'>ℹ️ Table 'client_queries' already exists. Checking structure...</div>";
    
    // Check if document_url column exists
    $check_column = $conn->query("SHOW COLUMNS FROM client_queries LIKE 'document_url'");
    
    if ($check_column->num_rows == 0) {
        echo "<div class='info'>Adding missing 'document_url' column...</div>";
        
        $alter_sql = "ALTER TABLE client_queries ADD COLUMN document_url VARCHAR(500) AFTER voice_url";
    ...
    echo "<div class='info'>Creating new 'client_queries' table...</div>";
    ...
    $sql = "CREATE TABLE client_queries (
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<div class='success'>✅ Table 'client_queries' created successfully!</div>";
        
        // Try to add foreign keys if tables exist
        $fk_added = 0;
        
        if ($companies_exists) {
            $fk_sql = "ALTER TABLE client_queries ADD CONSTRAINT fk_company FOREIGN KEY (company_id) REFERENCES company(company_id) ON DELETE CASCADE";
        ...
        if ($users_exists) {
            $fk_sql = "ALTER TABLE client_queries ADD CONSTRAINT fk_created_by FOREIGN KEY (created_by) REFERENCES user(user_id) ON DELETE CASCADE";
            if ($conn->query($fk_sql) === TRUE) {
...
$result = $conn->query("DESCRIBE client_queries");
...
echo "<div class='code'>";
echo htmlspecialchars("CREATE TABLE client_queries (
    query_id INT AUTO_INCREMENT PRIMARY KEY,
    company_id INT NOT NULL,
    client_name VARCHAR(255) NOT NULL,
    qa_pairs JSON NOT NULL COMMENT 'Array of {question, answer} objects',
...
echo "<p>The client_queries table is ready to use.</p>";
